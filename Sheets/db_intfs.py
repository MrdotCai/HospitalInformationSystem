...
def doctor_insert_value(name, sex, tel, email, department, user_key):
    "新建医生项，参数均为字符串格式，sex一定要传'm'或者'f'"
    depa_obj = Department.objects.filter(name=department).first()
    Doctor.objects.create(name=name, sex=sex, tel=tel, email=email, department=depa_obj, user_key=user_key)

def patient_insert_value(name, sex, tel, email, user_key):
    "新建患者项，参数均为字符串格式"
    Patient.objects.create(name=name, sex=sex, tel=tel, email=email, user_key=user_key)
...
def medi_insert_value(doctor_id, patient_id, submit_time, appo_time, 
    ...
    MedicalRecord.objects.create(appo_confirm=False, appo_cancel=False, appo_complete=False,
...
def create_new_doctor(username, password, name, sex, tel, email, department):
    ...
    doctor_insert_value(name, sex, tel, email, department, user.pk)

def create_new_patient(username, password, name, sex, tel, email):
    "创建患者用户，所有参数均为字符串类型，sex请传'f'或'm'"
    user = User.objects.create_user(username=username, password=password)
    patient_insert_value(name, sex, tel, email, user.pk)
...
def pat_ask_history(patient_id):
    ...
    for record in handled_records:
        ...
        tmp['appo_message'] = record.appo_message
        # 获取诊断信息
        tmp['disease'] = record.disease
        tmp['detail'] = record.detail
        tmp['diag_message'] = record.diag_message
